...
Encoder::Encoder(const EncoderHardwareConfig_t& hw_config,
                 Config_t& config, const Motor::Config_t& motor_config) : hw_config_(hw_config),
                                                                          config_(config) {
    update_pll_gains();
...
bool Encoder::run_offset_calibration() {
    ...
    else if (axis_->motor_.config_.motor_type == Motor::MOTOR_TYPE_BRUSHED) {// Brushed motor initialize of encoder
        config_.offset = 0;
